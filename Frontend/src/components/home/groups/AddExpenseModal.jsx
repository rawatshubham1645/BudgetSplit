...
const schema = yup.object().shape({
  ...
  groupId: yup.string().when('$showGroupSelect', { is: true, then: yup.string().required('Group is required') })
});
...
export default function AddExpenseModal({
  ...
  const {
    ...
    defaultValues: { name: "", description: '', amount: '', payer: currentUser?.id || "", groupId: '', date: new Date().toISOString().split('T')[0], // 👈 today's date in 'YYYY-MM-DD' format 
    participants: [] }
  });

  const [members, setMembers] = useState([])

  const { fields, update, replace } = useFieldArray({ control, name: 'participants' });
  ...
  useEffect(() => {
    if (members.length) {
      const parts = members.map(u => ({ userId: u.id, name: `${u.firstName} ${u.lastName}`, include: false, percentage: 0 }));
      replace(parts);
    ...
    ? (groupOptions?.find((el) => el.id == selectedGroupId)?.usersList?.map((user) => {
        if(user.id == currentUser.id){
            return {id: user.id, label: `${user.firstName} ${user.lastName} (You)`}
        } else {
  ...
  useEffect(() => {
    if (members.length) {
      const parts = members.map(u => ({ userId: u.id, name: `${u.label}`, include: false, percentage: 0 }));
      replace(parts);
    }
  }, [members, replace]);
  ...
  const handleSplitEqually = () => {
    ...
    fields.forEach((_, idx) =>
      setValue(`participants.${idx}.percentage`, watchedParticipants[idx].include ? share : 0)
    );
  };
  ...
  const onSubmit = async (data) => {
    ...
    const response = await mutate({ url: `api/expenses/add`, method: 'POST', data: {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          {showGroupSelect && (
            <div>
              ...
              {errors.groupId && <p className="mt-1 text-sm text-red-500">{errors.groupId.message}</p>}
            </div>
          )}
          ...
          <div>
            ...
            {errors.payer && <p className="mt-1 text-sm text-red-500">{errors.payer.message}</p>}
          </div>
                ...
                return (
                  <div key={field.id} className="flex items-center gap-2">
                    <input
                      type="checkbox"
                      checked={include}
                      onChange={e => setValue(`participants.${idx}.include`, e.target.checked)}
                      className="h-4 w-4 text-indigo-600 border-gray-300 rounded"
                    />
                    <span className="flex-1 text-sm text-gray-800 dark:text-gray-200">{field.name}</span>
                    <Input
                      type="number"
          ...
          <Button type="submit" className="w-full py-3 font-semibold rounded-lg">
            Add Expense
          </Button>
        </form>
      </motion.div>
